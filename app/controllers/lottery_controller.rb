class LotteryController < ApplicationController
  ...
  def run_intelligent_assignment
    ...
    begin
      ...
      team_to_topic_info = create_new_teams_for_bidding_response(teams, assignment, priority_info)
      match_new_teams_to_topics(assignment, team_to_topic_info)
  ...
  def create_new_teams_for_bidding_response(teams, assignment, priority_info)
    # Holds a map between team id and list of topics the team prefers
    # For example below, team id 3  prefers topic 2 and then topic 3
    # {
    #   1: [1, 4, 3, 5],
    #   2: [0, 1, 2],
    #   3: [2, 3],
    #   4: [2, 0, 1]
    # }
    # This map is returned at the end of this method
    topic_priority_dict = Hash.new
    ...
    teams.each do |user_ids|
      ...
      topic_priority_info = merge_bids_from_different_previous_teams(assignment, new_team.id, user_ids, priority_info)
      topic_priority_dict[new_team.id] = topic_priority_info
    end
    # Remove empty teams
  ...
  def merge_bids_from_different_previous_teams(assignment, team_id, user_ids, priority_info)
    ...
    # Returns the list of topics that the team bid for in order of priority
    return bidding_matrix_summary.collect{|b| b[2]}
  end
  ...
  def match_new_teams_to_topics(assignment, team_to_topic_info)
    # Takes in an assignment and a Hash that maps teams to an array that lists the teams topic
    # preferences in order and then attempts to match all teams to their preferred topics
    ...
    unassigned_teams.each do |team|
      if team_to_topic_info.key?(team.id)
        team_bids << team_to_topic_info[team.id]
        next
      end
      topic_bids = []
